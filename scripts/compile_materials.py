...
def parse_program_options():
    ...
    argparser.add_argument('materials',
                           help='list of materials\' names or directories to compile (e. g. \'color-debug.json\')',
                           nargs='+')
    argparser.add_argument('-s', '--shaders-src-folder', dest='shaders_src_folder', default='../contents/shaders',
                           help='shaders source files folder (default is \'../contents/shaders\')',
                           metavar='<shaders-source-folder>')
    argparser.add_argument('-i', '--shaders-include-folder', dest='shaders_include_folder',
                           default='../contents/shaders/include',
                           help='shaders include source files folder (default is \'../contents/shaders/include\')',
                           metavar='<shaders-include-folder>')
    argparser.add_argument('-S', '--materials-src-folder', dest='materials_src_folder', default='../contents/materials',
                           help='materials source files folder (default is \'../contents/materials\')',
                           metavar='<materials-source-folder>')
    argparser.add_argument('-o', '--output-path', dest='outpath', default='../contents/shaders/bin',
                           help='path to folder where to save compiled shaders (default is \'../contents/shaders/bin\')',
                           metavar='<output-path>')
...
def get_shader_compilation_flags(shader_module: ShaderModuleInfo) -> list:
...
def get_shader_preprocessor(glsl_preprocessor: GLSLShaderPreprocessor, hlsl_preprocessor: HLSLShaderPreprocessor,
                            shader_module: ShaderModuleInfo) -> AbstractShaderPreprocessor:
    if shader_module.shader_language == ShaderLanguage.GLSL:
...
def compile_material(program_options: dict, material_data: dict):
